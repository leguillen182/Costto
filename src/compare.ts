...
export function compareBoqs(boqA: Boq, itemsA: BoqItem[], boqB: Boq, itemsB: BoqItem[]): CompareResult {
  ...
  for (const k of new Set([...mapA.keys(), ...mapB.keys()])) {
    ...
    const deltaAmount = amountA != null && amountB != null ? round(amountB - amountA) : null;
    const deltaPct = deltaAmount != null && amountA ? round((deltaAmount / amountA) * 100) : null;
  ...
  return {
    ...
    totalA: round(totalA),
    totalB: round(totalB),
    deltaTotal: round(totalB - totalA),
